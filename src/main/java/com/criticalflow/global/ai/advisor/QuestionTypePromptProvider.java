        ...
        return switch (type) {
            ...
            case TYPE_C -> """
                    TYPE C — 과거 학습 연계 (Spaced Recall)
                      ...
                      [RAG 활용 지침]
                      - Retrieved Past Context에 포함된 note 제목과 핵심 개념을 질문에 명시적으로 언급한다.
                      - 두 개념의 관계 유형을 먼저 파악한 뒤 아래 템플릿 중 하나를 선택한다:
                        · 확장 (A → B 심화):
                          "전에 [A]를 배웠는데, 지금 [B]는 어떤 점에서 그걸 넘어서는 것 같아?"
                        · 대조 (A vs B):
                          "[A]와 [B]는 같은 문제를 다른 방식으로 해결해. 어떤 상황에서 B를 골라야 할까?"
                        · 기반 (A가 B의 토대):
                          "[A]가 없다면 [B]가 어떻게 달라질 것 같아?"
